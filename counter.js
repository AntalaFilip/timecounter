...
class Counter {
	...
	get realDiff() { return Math.abs(Date.now() - this._elapsed); }
	/** how many TRUE ms since last data update */
	get trueDiff() { return this.realDiff * this.speedModifier; }
	...
	get trueDate() { return new Date(this.trueMs + this.startPoint); }
	...
	save(bc = true) {
		...
		this._elapsed = Date.now();
		if (bc) this.io.to(this.name).emit('stateChange', this.name, this.metadata());
	...
	statusMessage() {
		...
		console.log(`True elapsed time before update: ${this._elapsed} milliseconds`);
	...
	static get(counter) { return counters.get(counter); }
}
